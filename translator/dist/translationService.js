...
export async function filterOutRealWords(words) {
    await loadDictionary();
    let realWords = words.filter(word => armenianDict.has(word));
    return realWords;
}
...
export async function translateWordAPI(sourceWord, sourceLang, resultLang) {
    try {
...
export async function translateWord(word) {
    ...
    const targetLang = detectedLang === 'hy' ? 'en' : 'hy';
